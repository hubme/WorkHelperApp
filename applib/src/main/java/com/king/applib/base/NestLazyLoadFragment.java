...
/**
 * https://github.com/ImportEffort/FragmentLiftCycle.
 * https://blog.csdn.net/learningcoding/article/details/80044942
 *
 * @author wangshijia
 */
public abstract class NestLazyLoadFragment extends BaseFragment {
...
//        // 当父 dispatchChildVisibleState 的时候第二次回调本方法 visible = false 所以此处 visible 将直接返回
    ...
    private boolean isParentInvisible() {
        ...
        if (parentFragment != null && parentFragment instanceof NestLazyLoadFragment) {
            NestLazyLoadFragment fragment = (NestLazyLoadFragment) parentFragment;
    ...
    private void dispatchChildVisibleState(boolean visible) {
        ...
        List<Fragment> fragments = childFragmentManager1.getFragments();
        if (!fragments.isEmpty()) {
            for (Fragment child : fragments) {
                if (child instanceof NestLazyLoadFragment && !child.isHidden() && child.getUserVisibleHint()) {
                    ((NestLazyLoadFragment) child).dispatchUserVisibleHint(visible);
                }
            }
        }
    }
