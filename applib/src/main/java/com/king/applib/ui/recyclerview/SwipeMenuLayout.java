...
public class SwipeMenuLayout extends ViewGroup {
    ...
    private int mScaleTouchSlop;//为了处理点击事件的冲突
    ...
    //滑动界值,超过一定距离后，手指抬起时，自动展开或关闭menu
    private int mLimit;

    private View mContentView;//内容布局(第一个View)
    ...
    //存储的是当前正在展开的View
    private static SwipeMenuLayout mOpeningMenuView;
    ...
    private void init(Context context) {
        ...
        mExpandAnim.setDuration(ANIMATION_DURATION);
        mExpandAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override public void onAnimationUpdate(ValueAnimator animation) {
                scrollTo((Integer) animation.getAnimatedValue(), 0);
            }
        });
        mExpandAnim.setInterpolator(new AccelerateDecelerateInterpolator());
        ...
        mCloseAnim = ValueAnimator.ofInt();
        mCloseAnim.setDuration(ANIMATION_DURATION);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mMenuWidth = 0;//由于ViewHolder的复用机制，每次这里要手动恢复初始值
        mHeight = 0;
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            childView.setClickable(true);//设置每一个子View可点击，从而获取触摸事件
            ...
            if (i > 0) {//第一个布局是item，从第二个开始才是RightMenu
        ...
        mLimit = (int) (mMenuWidth * DEFAULT_THRESHOLD);
        ...
        if (isSwipeEnable) {
            ...
            switch (ev.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    isUserSwiped = false;
                    isUnMoved = true;
                    iosInterceptFlag = false;//每次DOWN时，默认是不拦截
                    ...
                    if (mOpeningMenuView != null) {
                        if (mOpeningMenuView != this) {
                            mOpeningMenuView.smoothClose();
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (Math.abs(gap) > mViewConfiguration.getScaledTouchSlop() || Math.abs(getScrollX()) > mViewConfiguration.getScaledTouchSlop()) {
                        getParent().requestDisallowInterceptTouchEvent(true);
                    }
    ...
    public static SwipeMenuLayout getViewCache() {
        return mOpeningMenuView;
    }

    private void cancelAnim() {
        if (mCloseAnim.isRunning()) {
            mCloseAnim.cancel();
        }
        if (mExpandAnim.isRunning()) {
            mExpandAnim.cancel();
        }
    }

    public void smoothExpand() {
        mOpeningMenuView = SwipeMenuLayout.this;
        ...
        cancelAnim();
        mExpandAnim.setIntValues(getScrollX(), isLeftSwipe ? mMenuWidth : -mMenuWidth);
        mExpandAnim.start();
    }
    
    public void smoothClose() {
        mOpeningMenuView = null;
        ...
        cancelAnim();
        mCloseAnim.setIntValues(getScrollX(), 0);
        mCloseAnim.start();
    }
    ...
    public void quickClose() {
        if (this == mOpeningMenuView) {
            cancelAnim();
            mOpeningMenuView.scrollTo(0, 0);//关闭
            mOpeningMenuView = null;
        }
    }
